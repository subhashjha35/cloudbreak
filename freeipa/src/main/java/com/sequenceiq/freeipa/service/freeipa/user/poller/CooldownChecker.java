...
class CooldownChecker {
    ...
    public boolean isCooldownExpired(UserSyncStatus userSyncStatus, Instant cooldownThresholdTime) {
        ...
        boolean cool;
        if (userSyncStatus.getLastRequestedFullSync() != null) {
            cool = Instant.ofEpochMilli(userSyncStatus.getLastRequestedFullSync().getStartTime()).isBefore(cooldownThresholdTime);
