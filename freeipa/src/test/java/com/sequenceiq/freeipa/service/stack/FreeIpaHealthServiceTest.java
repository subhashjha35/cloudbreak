...
public class FreeIpaHealthServiceTest {
    ...
    private static FreeIpaClientException ipaClientExceptionWrongMaster;
    ...
    public static void init() {
        ...
        ipaClientExceptionWrongMaster = new FreeIpaClientException("invalid 'cn': must be \"" + HOST2 + "\"");
    }

    @BeforeEach
    public void setUp() {
        ReflectionTestUtils.setField(underTest, "maxRetries", 3);
    }
    ...
    @Test
    public void testWrongMaster() throws Exception {
        FreeIpaClient mockIpaClient = Mockito.mock(FreeIpaClient.class);
        Mockito.when(stackService.getByEnvironmentCrnAndAccountIdWithLists(anyString(), anyString())).thenReturn(getGoodStackTwoInstances());
        Mockito.when(freeIpaClientFactory.getFreeIpaClientForStack(any())).thenReturn(mockIpaClient);
        Mockito.when(mockIpaClient.serverConnCheck(eq(HOST2), eq(HOST1))).thenReturn(getGoodPayload(HOST1));
        Mockito.when(mockIpaClient.serverConnCheck(eq(HOST2), eq(HOST2))).thenReturn(getGoodPayload(HOST2));
        Mockito.lenient().when(mockIpaClient.serverConnCheck(eq(HOST1), anyString())).thenThrow(ipaClientExceptionWrongMaster);
        HealthDetailsFreeIpaResponse response = underTest.getHealthDetails(ENVIRONMENT_ID, ACCOUNT_ID);
        Assert.assertEquals(Status.AVAILABLE, response.getStatus());
        Assert.assertTrue(response.getNodeHealthDetails().size() == 2);
        for (NodeHealthDetails nodeHealth:response.getNodeHealthDetails()) {
        ...
        Assert.assertTrue(response.getNodeHealthDetails().stream().filter(nodeHealthDetails -> nodeHealthDetails.getName().equals(HOST1)).count() == 1);
        Assert.assertTrue(response.getNodeHealthDetails().stream().filter(nodeHealthDetails -> nodeHealthDetails.getName().equals(HOST2)).count() == 1);
    }
